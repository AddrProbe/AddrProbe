...
def get_fre_pattern(df_prefix_hit_alias):
    all_pattern_string = df_prefix_hit_alias["cluster_distribution"].values.tolist()
    # print(all_pattern_string)
    all_pattern_list = []
    for str_pattern in all_pattern_string:
        pattern = ast.literal_eval(str_pattern)
        if pattern != [[0]]:
    ...
    df_counts.insert(0, 'id', temp_column )
...
def get_no_match_prefix(df_prefix_hit_alias, df_counts):
    ...
    df_all_as_attr = pd.read_csv('../data/input/all_as_attr.csv',low_memory=False)[['as',"org_name",'Category 1 - Layer 1','Category 1 - Layer 2']]
    df_all_as_attr = df_all_as_attr.rename(columns={'Category 1 - Layer 1': 'attr1'})
    df_all_as_attr = df_all_as_attr.rename(columns={'Category 1 - Layer 2': 'attr2'})

    df_no_match_prefix_attr_all = pd.merge(df_no_match_prefix_attr,df_all_as_attr, how='left')
    df_no_match_prefix_attr_all.to_csv(config.get_fre_pattern_path + "no_match_prefix_as_attr.txt", index = False)


    # get frequent pattern attr
    df_id_prefix_as_attr = pd.read_csv(config.model_prefix_attr_path, sep = ',')
    df_counts_as = pd.merge(df_counts,df_id_prefix_as_attr,on = ['id'], how='left')
    df_counts_attr = pd.merge(df_counts_as,df_all_as_attr,on = ['as'], how='left')
    # print(df_counts_attr)
    df_counts_attr.to_csv(config.get_fre_pattern_path + "no_match_knowbase_prefix_as_attr.txt", index = False)



def no_mathch_pre():
    path = config.get_fre_pattern_path + 'prefix_hit_alias_info.txt'
    df_prefix_hit_alias = pd.read_csv(path)
    df_counts = get_fre_pattern(df_prefix_hit_alias)
    get_no_match_prefix(df_prefix_hit_alias, df_counts)
...
def get_match_model_cluster_distribution(prefix, df_all_test_prefix, df_id_prefix_as_attr, prefix_cluster_distribution_dict, budget):
    test_prefix_attr = df_all_test_prefix[df_all_test_prefix['id'] == prefix].iloc[0,:]
    ...
    elif len(mathch_model_org) > 5:
        mathch_model_org = df_id_prefix_as_attr[(df_id_prefix_as_attr['attr1'] == attr1) & (df_id_prefix_as_attr['attr2'] == attr2)][['id', 'prefix']].values.tolist()
        if len(mathch_model_org) == 0:
            mathch_model_org = df_id_prefix_as_attr[df_id_prefix_as_attr['attr1'] == attr1][['id', 'prefix']].values.tolist()

    if len(mathch_model_org) >5:
        mathch_model_org = random.sample(mathch_model_org, 5)
...
def init_probe_all_model():
    ...
    df_all_test_prefix = pd.read_csv(config.get_fre_pattern_path + "no_match_prefix_as_attr.txt", sep = ',')
    df_id_prefix_as_attr = pd.read_csv(config.get_fre_pattern_path + "no_match_knowbase_prefix_as_attr.txt", sep = ',')
    ...
    prefix_list= df_all_test_prefix["id"].to_list()
    for prefix in prefix_list:
