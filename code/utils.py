...
class IterAddressDataset(Dataset):
    def __init__(self, prefix_index, cluster_num, train_data):
        self.prefix_index = prefix_index
        self.cluster_num = cluster_num
        self.train_data = train_data
        # print(self.prefix_index)
        ...
        # self.path = config.new_address_path + f"{self.prefix_index}.txt"
        # self.data_with_label = np.loadtxt(self.path, delimiter=',', dtype = str)
        self.data_with_label = self.train_data   
        # print(self.data_with_label)
        # if self.data_with_label.shape[0] > 100000:
        #     self.data_with_label = self.data_with_label[np.random.randint(self.data_with_label.shape[0], size=100000), :]
...
def load_iter_train_data(prefix_index, cluster_num, train_data):
    dataset = IterAddressDataset(prefix_index, cluster_num, train_data)
...
def copy_model(prefix_index):
    ...
    target_path = config.iter_model_path + "{prefix_index}.pth".format(prefix_index=prefix_index)
...
def alias_detection(df_active_address_label, prefix_index, prefix):
    len_prefix = int(int(re.findall('/(.*)', prefix)[0]) / 4)
    df_active_address_label = df_active_address_label["address"].str.replace(':', '').astype(str).to_frame()

    list_all = []
    for index, row in df_active_address_label.iterrows():
        list_temp = list(row["address"])
        list_all.append(list_temp)
    df_active_address_label = pd.DataFrame(data=list_all)

    entro_list = []
    count_list = []
    for i in range(32):
        per_entro = round(pandas_entropy(df_active_address_label.iloc[:, i], base=None), 2)
        entro_list.append(per_entro)
        count = df_active_address_label.iloc[:, i].value_counts()
    ...
    entro_diff_max = len_prefix 
    for en in entro_diff:
        temp_diff_max = entro_diff.index(en) + len_prefix
        if en > en_values and temp_diff_max < 30:
    ...
    cmd2 = f'sudo -S zmap --ipv6-source-ip={config.local_ipv6} --ipv6-target-file=' + df_path + ' -o ' + config.zmap_result_path + 'scan_' + zmap_file + ' -M icmp6_echoscan -B 10M --verbosity=0'
    echo = subprocess.Popen(['echo',config.password], stdout=subprocess.PIPE,)
    p = subprocess.Popen(cmd2, shell=True, stdin=echo.stdout, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
