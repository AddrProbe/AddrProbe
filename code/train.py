...
def iter_train_specific_prefix(prefix, model_id, cluster_num, train_data, lr=config.lr * 0.1, epoch_num=config.epoch_num):
    ...
    train_loader = load_iter_train_data(prefix, cluster_num, train_data)
    cluster_num = train_loader.dataset.get_cluster_num()
    model = CVAE(cluster_num).to(device)
    model.load_state_dict(torch.load(config.iter_model_path + f"{model_id}.pth"))
    train(train_loader, model, model_id, lr=lr, epoch_num=epoch_num)
